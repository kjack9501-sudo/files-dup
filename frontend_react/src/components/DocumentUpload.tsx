  ...
  const handleFile = async (file: File) => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        onDocumentUploaded(data.documentId, data.filename);
      } else {
        setError(data.error || 'Failed to upload document. Please try again.');
      }
    } catch (err) {
      setError('Failed to upload document. Please check if the API server is running.');
      console.error(err);
    } finally {
      setUploading(false);
    }
